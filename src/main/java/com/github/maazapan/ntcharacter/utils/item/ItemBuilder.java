package com.github.maazapan.ntcharacter.utils.item;
...
public class ItemBuilder {
    ...
    public ItemBuilder setLore(String... list) {
        ...
        for (String s : list) {
            lore.add(KatsuUtils.coloredHex(s));
        }
    ...
    public ItemBuilder fromConfig(FileConfiguration config, String path) {
        ...
        if (config.contains(path + ".skull")) {
            this.setSkullBase64(config.getString(path + ".skull"));
        }
        ...
        if (config.isSet(path + ".actions")) {
            nbtItem.setObject("character-actions", config.getStringList(path + ".actions"));
